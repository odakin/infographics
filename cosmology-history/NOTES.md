...
- Plot の右上 label「加速膨張開始」「DE 優勢」 が物理的近接 (a = 0.59 vs 0.75) で接近気味。 connector line で引き出す方が綺麗かも
- 各段の icon、 特にステージ 4 (天秤) と 5 (再結合) はさらに洗練可能
- responsive breakpoint をもっと細かく (= 1280px / 1024px 等)
- アニメーション (= 各段の hover で詳細展開、 plot の line drawing animation 等)
