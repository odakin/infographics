...
## LaTeX 移行 (= 2026-05-19 第 3 版、 primary)

HTML 版は user feedback で複数の構造的問題が指摘された:
- 赤方偏移式 (KaTeX) が clip
- font が OS 依存でダサい
- 絵・テーブルの揃えが甘い
- 右下の DE 優勢 annotation が見切れる

これらは HTML/CSS の以下の原理的弱点に起因:
1. CSS layout と KaTeX 計測の race condition で数式が clip しやすい
2. Web font は OS/Renderer (Chromium vs Safari vs WebKit) で字形 hinting が違う、 print PDF も font subset embed 制御が CSS では弱い
3. CSS Grid は「だいたい揃える」 の言語、 ベースライン整列や 1mm 単位の絶対配置が苦手
4. browser print pipeline で `mm` 指定しても 0.5mm 程度のズレが出る

→ **LuaLaTeX + TikZ + pgfplots に primary 移行**。 解消:
- 数式: LaTeX native で絶対 clip しない
- font: Hiragino Mincho/Sans を subset embed、 print fidelity 保証
- 配置: TikZ 絶対座標で 1mm 単位、 baseline anchor で確実整列
- annotation 見切れ: plot 内座標 (axis cs) で確実配置

HTML 版は **web 配布の代替** として残置 (= link で共有可能、 PDF download 不要の用途)。

build: `cd cosmology-history && make` (= LuaLaTeX 一発で PDF 生成)、 `make png` で 300 DPI PNG 出力 (= chat 共有用)。

## レイアウト・プロット design (= 2026-05-19 第 2 版、 HTML 版の最終形)
